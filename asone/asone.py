...
class ASOne:
    ...
    def track_stream(self,
        ...
        for (bbox_details, frame_details) in self._start_tracking(stream_url, self.configss):
    ...
    def detect_video(self,
        ...
        fps = self.configss.pop('fps')
        output_dir = self.configss.pop('output_dir')
        filename = self.configss.pop('filename')
        save_result = self.configss.pop('save_result')
        display = self.configss.pop('display')
        draw_trails = self.configss.pop('draw_trails')
        class_names = self.configss.pop('class_names')
    ...
    def track_webcam(self,
        ...
        for (bbox_details, frame_details) in self._start_tracking(cam_id, self.configss):
            # yeild bbox_details, frame_details to main script
            yield bbox_details, frame_details
    ...
    def _start_tracking(self,
        ...
        while True:
            ...
            cv2.putText(im0, f'FPS: {int(fps)}', (11, 35), 0, 1, [
                        225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
            # cv2.putText(im0, f'Total: {int(len(class_ids))}', (60, 800),
            #         cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
            # cv2.putText(im0, f'Male: {int(nMale)}', (60, 900),
            #         cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
            # cv2.putText(im0, f'Female: {int(nFemale)}', (60, 1000),
            #         cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 2)
